# -*- coding: utf-8 -*-
"""7-master_dataset_processing.ipynb
...
master_df.drop(['HEAT_DEG_DAYS', 'COOL_DEG_DAYS'], axis=1, inplace=True)
...
sample_df.to_csv('test.csv.gz', compression='gzip', index=False)
files.download('test.csv.gz')
...
for temp_label in ['MEAN_TEMP', 'MAX_TEMP', 'MIN_TEMP', 'TEMP_12_4', 'DEW_POINT_TEMP_12_4']:
  sample_df['QUANTILE_'+temp_label] = pd.qcut(sample_df[temp_label], q=8, precision=0, duplicates='drop')

# Bin precipitation, rain
for temp_label in ['TOTAL_RAIN', 'TOTAL_PRECIP']:
  sample_df['QUANTILE_'+temp_label] = pd.qcut(sample_df[temp_label], q=5, precision=0, duplicates='drop')

# Bin relative humidity
sample_df['QUANTILE_REL_HUM_12_4'] = pd.qcut(sample_df['REL_HUM_12_4'], q=10, precision=0, duplicates='drop')

# Bin absolute humidity
sample_df['QUANTILE_ABS_HUM_12_4'] = pd.qcut(sample_df['ABS_HUM_12_4'], q=5, precision=0, duplicates='drop')

sample_df

sample_df.to_csv('processed_sample_dataset.csv.gz', compression='gzip')

# Save file to google drive
file_path = root_path + f'Colab Data/{province}_processed_sample_dataset.csv.gz'
shutil.copy('processed_sample_dataset.csv.gz', file_path)